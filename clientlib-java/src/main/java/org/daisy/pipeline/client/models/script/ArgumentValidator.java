...
public class ArgumentValidator {
	 ...
	 */
	public static String validate(Argument arg, Context context) {
				...
				case "anyURI":
					try {
						...
						if ("".equals(arg.output)) { // input type => validate that it refers to context files
							...
							if (context != null && !context.exists(value)) {
				...
				case "anyDirURI":
					try {
						...
						if ("".equals(arg.output)) { // input type => validate that it refers to context files
							...
							if (context != null && !context.isDirectory(value)) {
								return beginText+" must refer to a directory that exists in the job context.";
							}
//							if (directory.isDirectory()) { // only validate presence of context files if job is stored to disk
//								File targetFile = new File(new File(directory, "context"), value);
				...
				case "anyFileURI":
					try {
						...
						if ("".equals(arg.output)) { // input type => validate that it refers to context files
							...
							if (context != null && !context.isFile(value)) {
								return beginText+" must refer to a directory that exists in the job context.";
							}
//							if (directory.isDirectory()) { // only validate presence of context files if job is stored to disk
//								File targetFile = new File(new File(directory, "context"), value);
