...
public class Pipeline2WSTest {
	...
	public void getScripts() {
		try {
			Pipeline2Client.setHttpClientImplementation(new MockHttpClient());
	...
	public void testParseAliveResponse() {
		try {
			Pipeline2Client.logger().setLevel(Pipeline2Logger.LEVEL.ALL);
			Pipeline2Client.setHttpClientImplementation(new MockHttpClient());
	...
	public void testParseJobRequest() {
		try {
			...
			for (Argument arg : script.arguments) {
				if ("source".equals(arg.name)) {
					assertEquals(true, "file1.xml".equals(arg.getFile(0)));
					assertEquals(true, "file2.xml".equals(arg.getFile(1)));
				}
