...
//		testFolder.delete();
	...
	public void testListAndLoadJobs() {
		List<JobStorage> jobs = JobStorage.listJobs(jobStorage);
		assertEquals(2, jobs.size());
		
		
		JobStorage job = jobs.get(0);
	...
	public void testDeleteJob() {
		...
		for (JobStorage job : JobStorage.listJobs(jobStorage)) {
			jobsBefore.add(job.getId());
		}
		
		JobStorage job2 = JobStorage.loadJob("job2", jobStorage);
		job2.delete();
	...
	public void testStoreJob() {
		...
		JobStorage newJob = JobStorage.createJob("job3", jobStorage, loadResource("scripts/dtbook-to-epub3.xml"));
	...
	public void testFiles() {
		JobStorage job = JobStorage.createJob("filesJob", jobStorage, loadResource("scripts/dtbook-to-epub3.xml"));
